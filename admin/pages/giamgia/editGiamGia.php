...
<?php 
                            $i = 1;
                                $query = $conn->query("SELECT * FROM `magiamgia` WHERE `id` = '$id'");
                                while($row1 = $query->fetch(PDO::FETCH_ASSOC)){
                                    //get info giảm giá có id trên
                                ?>
<!-- partial -->
<div class="main-panel" style="padding:20px">
    <div class="box">
        <div class="box-header with-border">
            <div class="left">
                <h3 class="box-title">Sửa giảm giá</h3>
            </div>
        </div><!-- /.box-header -->
        ...
        <form class="box-body" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="control-label">Mã giảm giá
                </label>
                <input type="text" class="form-control" name="title" placeholder="Mã giảm giá"
                    value="<?php echo ''.$row1["magiam"]?>" id="title" fdprocessedid="oboy0m">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="soluong" placeholder="Số lượng"
                    value="<?php echo ''.$row1["soluong"]?>" id="soluong" fdprocessedid="oboy0m">
            </div>
            <div class="form-group">
                <label class="control-label">Số tiền giảm</label>
                <input type="text" id="sotien" class="form-control" name="sotien" placeholder="Số tiền giảm"
                    value="<?php echo ''.$row1["sotien"]?>" required>
            </div>
...
$sql = "UPDATE magiamgia SET `magiam`= '$title',`sotien`= '$sotien',`soluong`='$soluong',`batdau`='$sotien' WHERE `id` = '$id'";
    if ($conn->query( $sql)) {
